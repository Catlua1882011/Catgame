...
-- Configuration setup using getgenv().config
getgenv().config = {
    ["Premium"] = {
        ["Key"] = ""  --// Premium Script
    },
...
-- User-defined key
local correctKeys = {"cat", "cat1"} -- Define valid premium keys

-- Check key and set status
if getgenv().key == "" then
    getgenv().Premium = false
    getgenv().Free = true
else
    getgenv().Premium = false
    for _, value in ipairs(correctKeys) do
        if getgenv().key == value then
            getgenv().Premium = true
            break
        end
    end
    getgenv().Free = not getgenv().Premium
end
...
if player.PlayerGui.Main:FindFirstChild("ChooseTeam") then
    repeat
        task.wait()
        if player.PlayerGui.Main.ChooseTeam.Visible then
            if getgenv().config.team == "Marines" then
                for _, v in pairs(getconnections(player.PlayerGui.Main.ChooseTeam.Container["Marines"].Frame.TextButton.Activated)) do
                    v.Function() -- Activate Marines team selection
                end
            else
                for _, v in pairs(getconnections(player.PlayerGui.Main.ChooseTeam.Container["Pirates"].Frame.TextButton.Activated)) do
                    v.Function() -- Activate Pirates team selection
                end
            end
        end
...
-- Ui
local Name = "Cắt tai Hub" .. (getgenv().Premium and " [Premium]" or getgenv().Free and " [Free]" or "")
local LoadingSubtitle = "- [Beta]"
...
local Tabs = {
    SettingTab = Window:AddTab({ Title = "Setting", Icon = "" }),
    chucnang = Window:AddTab({ Title = "Tets New function ", Icon = "" }),
    ...
    Player = Window:AddTab({ Title = "Player-Local Player", Icon = "" }),
    Fruit = Window:AddTab({ Title = "Fruit", Icon = "" }),
    Travel = Window:AddTab({ Title = "Travel", Icon = "" }),
    RaidMaterial = Window:AddTab({ Title = "Raid-Material", Icon = "" }),
    RaceV4Mirage = Window:AddTab({ Title = "RaceV4-Mirage", Icon = "" }),
...
local cooldowntickFire = 1
local MaxFire = 100000000
...
--//fastv2
-- Bỏ qua hoạt ảnh tấn công nếu cần
...
-- Ghi đè hàm wrapAttackAnimationAsync để quản lý hoạt ảnh
RL.wrapAttackAnimationAsync = function(a, b, c, d, func)
    if not NoAttackAnimation then
        return oldRL(a, b, c, 60, func)
    end
    ...
    -- Thu thập các kẻ thù trong khoảng cách tấn công
    local function collectHits(targets)
    ...
    collectHits(game:GetService("Workspace").Characters:GetChildren())
    collectHits(game:GetService("Workspace").Enemies:GetChildren())

    a:Play(0.01, 0.01, 0.01)
    pcall(func, Hits)  -- Gọi hàm với các mục tiêu đã thu thập
end

-- Hàm lấy tất cả các đòn tấn công bằng lưỡi dao
getAllBladeHits = LPH_NO_VIRTUALIZE(function(Sizes)
    local Hits = {}
    local Client = game.Players.LocalPlayer
...
-- Hàm lấy tất cả các đòn tấn công vào người chơi
getAllBladeHitsPlayers = LPH_NO_VIRTUALIZE(function(Sizes)
    local Hits = {}
    local Client = game.Players.LocalPlayer
...
-- Hàm hủy bỏ thời gian hồi chiêu
CancelCoolDown = LPH_JIT_MAX(function()
    local ac = CombatFrameworkR.activeController
    if ac and ac.equipped then
        ...
        -- Giảm FireL sau thời gian delay
...
AttackFunction = LPH_JIT_MAX(function(typef)
    ...
    if ac and ac.equipped then
        ...
        if #bladehit > 0 then
            pcall(task.spawn, ac.attack, ac)  -- Thực hiện tấn công
            ...
            task.delay(0, function()
                StartP:Stop()  -- Dừng hoạt ảnh sau khi tấn công
            end)
        end
    end
end)

-- Hàm kiểm tra trạng thái choáng
function CheckStun()
    if game:GetService('Players').LocalPlayer.Character:FindFirstChild("Stun") then
        return game:GetService('Players').LocalPlayer.Character.Stun.Value ~= 0
    end
...
-- Hàm tìm kiếm công cụ trong ba lô hoặc nhân vật
    ...
    if hookfunction and not islclosure(hookfunction) then 
        ...
        task.spawn(function()
            ...
            repeat 
                ...
                task.wait(1.5)  -- Chờ một khoảng thời gian trước khi thử lại
            until NGU 
        end) 
        ...
        task.spawn(function()
            ...
            -- Lưu trữ các hàm gốc để khôi phục nếu cần
            ...
            if abc then
                pcall(function()
                    c.wrapAttackAnimationAsync = function(d, e, f, g, h)
                        ...
                        if i then
                            -- Tạm thời vô hiệu hóa hàm play
                            b.play = function() end
                            d:Play(0.1, 0.1, 0.1)
                            h(i)  -- Gọi hàm h với các mục tiêu đã thu thập
                            b.play = shared.cpc  -- Khôi phục hàm play
                            wait(0.5)  -- Chờ trước khi dừng hoạt ảnh
                            d:Stop()  -- Dừng hoạt ảnh
                        end
                    end
                end)
            end
...
------//Select Weapon
local Dropdown = Tabs.SettingTab:AddDropdown("DropdownFarm", {
    Title = "Select Weapon",
...
local SelectFastAttackMode = (SelectFastAttackMode or "Fast V2")

SelectedFastAttackMode = {"Normal Attack","Fast Attack","Fast V2"}
...
local function ChangeModeFastAttack(SelectFastAttackMode)
	...
	elseif SelectFastAttackMode == "Fast Attack" then
		FireCooldown = 0.07
	elseif SelectFastAttackMode == "Fast V2" then
		FireCooldown = 0.02
	end
end

local SelectedFastAttackModes = Tabs.SettingTab:AddDropdown("SelectedFastAttackModes", {
...
local FASTAT = Tabs.SettingTab:AddToggle("Fast_Attack", {Title = "Fast Attack", Default = true })
...
local Toggle = Tabs.SettingTab:AddToggle("Bring Mob", { Title = "Bring Mob", Default = true })
...
local Toggle = Tabs.SettingTab:AddToggle("Turn V4", { Title = "Auto Active Race V4", Default = false })
Toggle:OnChanged(function(Value)
    RaceNguLon = Value
end)
...
--//ConCacDiMeChungMay
local Toggle = Tabs.SettingTab:AddToggle("Bypass TP", { Title = "Bypass TP", Default = false })
Toggle:OnChanged(function(Value)
    BypassTP = Value
end)

local Toggle = Tabs.SettingTab:AddToggle("Bypass TP", { Title = "Stop TP Bypass When Have Legendary", Default = false })
Toggle:OnChanged(function(Value)
    _G.StopTP = Value
end)
...
---//MainFarm
local Dropdown = Tabs.MainFarm:AddDropdown("DropdownFarm", {
...
local Toggle = Tabs.MainFarm:AddToggle("Start Farm", { Title = "Start Farm", Description = "Bắt Đầu Farm", Default = false })
Toggle:OnChanged(function(Value)
    _G.AutoFarm = Value
    StopTween(_G.AutoFarm)
end)

local Toggle = Tabs.MainFarm:AddToggle("Auto Spawn Boss Kata", { Title = "Auto Spawn Boss Cake Prince", Default = true })
Toggle:OnChanged(function(Value)
...
--// StackAutoFarm
local Sea1 = Tabs.StackAutoFarm:AddSection("Sea: 1 Fam")
local Toggle = Tabs.StackAutoFarm:AddToggle("Auto Saber", { Title = "Auto Saber[Full beta]", Default = false })
Toggle:OnChanged(function(Value)
    _G.Cuttay = Value
    StopTween(_G.Cuttay)
end)
                          ...
                          topos(CFrame.new(-1610.00757, 11.5049858, 164.001587))
                                ...
                                if game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("ProQuestProgress","SickMan") ~= 0 then
                                    ...
                                    if game:GetService("Workspace").Enemies:FindFirstChild("Mob Leader") or game:GetService("ReplicatedStorage"):FindFirstChild("Mob Leader") then
										topos(CFrame.new(-2876.59, 7.91, 5437.74))
...
local Toggle = Tabs.StackAutoFarm:AddToggle("Auto Pole", { Title = "Fam Pole", Default = false })
...
local Sea2 = Tabs.StackAutoFarm:AddSection("Sea: 2 Fam") 
local Toggle = Tabs.StackAutoFarm:AddToggle("Auto Rengoku", {
	Title = "Auto Factory",
	Default = false
})
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.StackAutoFarm:AddToggle("Auto Ectoplasm", {
	Title = "Fam Ectoplasm",
	Default = false
})
Toggle:OnChanged(function(Value)
...
--//Farm Sea 3 
local Sea3 = Tabs.StackAutoFarm:AddSection("Sea: 3 Fam")
local Toggle = Tabs.StackAutoFarm:AddToggle("Auto Eliter Hunter", { Title = "Auto Eliter Hunter", Default = false })
Toggle:OnChanged(function(Value)
    _G.Eli_Ngu = Value
game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest")
...
spawn(function()
	...
	local Toggle = Tabs.StackAutoFarm:AddToggle("Auto Eliter Hunter", { Title = "Auto Raid Pirates", Default = false })
Toggle:OnChanged(function(Value)
    _G.RaidPirate = Value
...
local Toggle = Tabs.StackAutoFarm:AddToggle("auto Kill", {
...
local Toggle = Tabs.StackAutoFarm:AddToggle("auto Kill", { Title = "Kill Boss Tushita", Default = false })
Toggle:OnChanged(function(Value)
...
Toggle = Tabs.StackAutoFarm:AddToggle("MyToggle", {Title = "Auto Yama", Default = _G.AutoYama })
    Toggle:OnChanged(function(Value)
        _G.AutoYama = Value
        end)
        spawn(function()
...
local Toggle = Tabs.StackAutoFarm:AddToggle("auto Change", {
	Title = "Change Haki",
	Default = false
})
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.StackAutoFarm:AddToggle("auto Change", { Title = "Auto Soul Reaper", Description = "", Default = false })
Toggle:OnChanged(function(Value)
_G.Buoi = Value
StopTween(_G.Buoi)
end)         
...
local Toggle = Tabs.StackAutoFarm:AddToggle("auto Change", { Title = "Auto Dough King + Farm Item", Description = "", Default = false })
Toggle:OnChanged(function(Value)
...
occhochest = Tabs.StackAutoFarm:AddToggle("occhochest", {Title = "Farm Chest ┊Tween", Default = AutoFarmChest})
...
Toggle = Tabs.StackAutoFarm:AddToggle("MyToggle", {Title = "Stop If Have Darkness Key & Cup", Default = _G.StopChest})
Toggle:OnChanged(function(Value)
    _G.StopChest = Value
end)
spawn(function()
...
if AnDepZai1 then
local Dropdown1 = Tabs.Travel:AddDropdown("DropdownFarm", {
    Title = "Select Island",
...
if AnDepZai2 then
local Dropdown2 = Tabs.Travel:AddDropdown("DropdownFarm", {
    Title = "Select Island",
...
if AnDepZai3 then
local Dropdown3 = Tabs.Travel:AddDropdown("DropdownFarm3", {
    Title = "Select Island",
...
local Toggle = Tabs.Travel:AddToggle("Start Tween", { Title = "Start Tween", Default = false })
Toggle:OnChanged(function(Value)
    _G.BayDao = Value     
...
local ConCac = Tabs.RaidMaterial:AddSection("Raid")
local Dropdown = Tabs.RaidMaterial:AddDropdown("DropdownFarm", {
...
local Toggle = Tabs.RaidMaterial:AddToggle("Auto Buy Chip", { Title = "Auto Buy Chip", Default = false })
Toggle:OnChanged(function(Value)
...
local ToggleNextIsland = Tabs.RaidMaterial:AddToggle("ToggleNextIsland", {Title = "Next Island", Default = false })
...
local Toggle = Tabs.RaidMaterial:AddToggle("Kill Aura", { Title = "Kill Aura", Default = false })
Toggle:OnChanged(function(Value)
    _G.Kill_Aura = Value
end)
...
local Toggle = Tabs.RaidMaterial:AddToggle("Kill Aura", { Title = "Start Raid", Default = false })
Toggle:OnChanged(function(Value)
...
local Mastery = Tabs.RaidMaterial:AddSection("Raid Misc")
...
local Toggle = Tabs.RaidMaterial:AddToggle("Kill Aura", { Title = "Auto Raid Law", Default = false })
Toggle:OnChanged(function(Value)
    _G.Raidlaw = Value
...
--//Fruit
local Toggle = Tabs.Fruit:AddToggle("Random Fruit", { Title = "Random Fruit", Default = false })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.Fruit:AddToggle("Teleport Fruit", { Title = "Teleport Fruit", Default = false })
Toggle:OnChanged(function(Value)
    _G.Tweenfruit = Value
...
local Toggle = Tabs.Fruit:AddToggle("Store Fruit", { Title = "Store Fruit", Default = true })
Toggle:OnChanged(function(Value)
...
--//RaceV4Mirage
Tabs.RaceV4Mirage:AddParagraph({
    Title = "Mirage"
})
-- Tạo Toggle cho AutoPullFuly
local AutoPullFulyToggle = Tabs.RaceV4Mirage:AddToggle("AutoPullFuly", 
{
    Title = "Auto Pull Fuly[Lock]", 
    Description = "",
    Default = false,
    Callback = function(Value)
        AutoPullFuly = Value -- Gán giá trị bật/tắt cho AutoPullFuly
    end
})

-- Bắt sự kiện khi trạng thái AutoPullFuly thay đổi
AutoPullFulyToggle:OnChanged(function(Value)
    AutoPullFuly = Value -- Cập nhật trạng thái AutoPullFuly theo giá trị Value
    spawn(function()
        while fask.wait(.5) do
            if AutoPullFuly then
                -- Kiểm tra điều kiện kéo cần gạt tại Temple of Time
                ...
                -- Kiểm tra MysticIsland và thời gian trong Lighting
                if game:GetService("Workspace").Map:FindFirstChild("MysticIsland") and (function()
                    ...
                    -- Điều hướng đến MysticIsland và kiểm tra các Part Neon
                    if (function()
                        if game:GetService("Workspace").Map:FindFirstChild("MysticIsland") then
                            for a, b in pairs(game:GetService("Workspace").Map:FindFirstChild("MysticIsland")
                                :GetDescendants()) do
                                if b.Name == 'Part' and b.Material == Enum.Material.Neon and b.Transparency == 0 then
                                    return true
                                end
                            end
                        end
                    ...
                    -- Kích hoạt kỹ năng và điều chỉnh camera hướng về phía mặt trăng
                ...
                -- Thông báo khi không phải đêm hoặc không tìm thấy MysticIsland
                local notif = Notification.new("success", "Cat hub[Notify]", "Can't Find Because not night or mirage")
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Teleport", { Title = "Teleport Mirage Island", Default = false })
Toggle:OnChanged(function(Value)
    _G.DaoBiLon = Value
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Teleport Gear", { Title = "Lock Moon", Default = false })
Toggle:OnChanged(function(Value)
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Teleport Gear", { Title = "Teleport Gear", Default = false })
Toggle:OnChanged(function(Value)
    _G.Luom_Gear = Value
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Teleport Gear", { Title = "Teleport Advanced Fruit", Default = false })
Toggle:OnChanged(function(Value)
    _G.RaDaoKhi = Value
...
local Race = Tabs.RaceV4Mirage:AddSection("All Race Trial")
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Auto Complete Trial", { Title = "Auto Train-Buy Gear", Default = false })
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Auto Complete Trial", { Title = "Auto Complete Trial", Default = false })
...
local Toggle = Tabs.RaceV4Mirage:AddToggle("Kill Trial", { Title = "Kill After Trial", Default = false })
Toggle:OnChanged(function(Value)
    _G.Kill_V4 = Value
...
local KiuTraiAl = Tabs.RaceV4Mirage:AddSection("Skill Kill After Trial")

local Toggle = Tabs.RaceV4Mirage:AddToggle("Kill Trial", { Title = "Skill Z", Default = false })
Toggle:OnChanged(function(Ditme)
    _G.ConCacZ = Ditme
end)

local Toggle = Tabs.RaceV4Mirage:AddToggle("Kill Trial", { Title = "Skill X", Default = false })
Toggle:OnChanged(function(Lonma)
    _G.ConCacX = Lonma
end)

local Toggle = Tabs.RaceV4Mirage:AddToggle("Kill Trial", { Title = "Skill C", Default = false })
Toggle:OnChanged(function(AnDepZai)
    ConCacC = AnDepZai
end)
--//SeaEvents                                 
local SeaEvents = Tabs.SeaEvents:AddSection("WAIT UPDATE")

--//Shop
local Toggle = Tabs.Shop:AddToggle("Random Bones", { Title = "Random Bones", Default = false })
...
--//chucnang
local Toggle = Tabs.chucnang:AddToggle("MyToggle", 
{
    Title = "Quest[Bone]", 
    Description = " ",
    Default = false,
    Callback = function(value)
        if value then
            StartQuest() -- Khi toggle bật
        else
            AbandonQuest() -- Khi toggle tắt
        end
    end 
})
...
spawn(function()
    while wait() do
        if BoneFMode == "Quest" and _G.Auto_Bone and AnDepZai3 then
            pcall(function()
                local player = game:GetService("Players").LocalPlayer
                local questGui = player.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text

                -- Nếu không có quest đang hoạt động, hủy quest và bắt đầu lại
                if not string.find(questGui, "Demonic Soul") then
                    AbandonQuest() -- Hủy nhiệm vụ hiện tại
                    wait(1) -- Thời gian chờ trước khi bắt đầu lại nhiệm vụ
                    StartQuest() -- Nhận nhiệm vụ mới
                end

                if not player.PlayerGui.Main.Quest.Visible then
                    UsefastattackPlayers = false
                    if BypassTP then
                        if (player.Character.HumanoidRootPart.Position - BoneQuestPos.Position).Magnitude > 1500 then
                            BTP(BoneQuestPos) -- Di chuyển đến vị trí quest
                        else
                            topos(BoneQuestPos) -- Di chuyển đến vị trí quest
                        end
                    else
                        topos(BoneQuestPos) -- Di chuyển đến vị trí quest
                    end
                    ...
                    if (BoneQuestPos.Position - player.Character.HumanoidRootPart.Position).Magnitude <= 3 then    
                        StartQuest() -- Gọi hàm bắt đầu quest
                    end
                else
                    HandleEnemies() -- Gọi hàm xử lý kẻ thù
                end
            end)
        end
    end
...
function StartQuest()
    print("Quest đã được bắt đầu!") -- Thông báo bắt đầu quest
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("StartQuest", "HauntedQuest2", 1) -- Gọi hàm bắt đầu quest
end

function AbandonQuest()
    print("Quest đã bị hủy!") -- Thông báo hủy quest
    game:GetService("ReplicatedStorage").Remotes.CommF_:InvokeServer("AbandonQuest") -- Gọi hàm hủy quest
end

function HandleEnemies()
    local enemies = game:GetService("Workspace").Enemies
    local targetEnemies = {"Reborn Skeleton", "Living Zombie", "Demonic Soul", "Posessed Mummy"}

    local allEnemiesDefeated = true -- Biến để kiểm tra xem tất cả kẻ thù đã bị đánh bại

    for _, enemyName in pairs(targetEnemies) do
        ...
        if enemy and enemy:FindFirstChild("HumanoidRootPart") and enemy:FindFirstChild("Humanoid") and enemy.Humanoid.Health > 0 then
            allEnemiesDefeated = false -- Có ít nhất một kẻ thù còn sống
            if string.find(game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Container.QuestTitle.Title.Text, "Demonic Soul") then
                attackEnemy(enemy) -- Gọi hàm tấn công kẻ thù
            end
        end
    end

    -- Nếu tất cả kẻ thù đã bị đánh bại, bắt đầu lại nhiệm vụ
    if allEnemiesDefeated then
        AbandonQuest() -- Hủy nhiệm vụ hiện tại
        wait(1) -- Thời gian chờ một chút trước khi bắt đầu lại nhiệm vụ
        StartQuest() -- Nhận lại nhiệm vụ mới
    end
end
...
function attackEnemy(enemy)
    repeat 
        ...
        NeedAttacking = true
        NameBoneMon = enemy.Name

        -- Di chuyển kẻ thù về vị trí cụ thể
        if enemy.Name == "Demonic Soul" then
            BringMobBone(enemy.Name, CFrame.new(-9497.908203125, 172.1048126220703, 6148.97119140625))
        elseif enemy.Name == "Living Zombie" then
            BringMobBone(enemy.Name, CFrame.new(-10143.466796875, 138.6266632080078, 5974.3330078125))
        elseif enemy.Name == "Reborn Skeleton" then
            BringMobBone(enemy.Name, CFrame.new(-8760.986328125, 142.1048126220703, 6039.1083984375))
        elseif enemy.Name == "Posessed Mummy" then
            BringMobBone(enemy.Name, CFrame.new(-9575.4267578125, 5.792530536651611, 6226.22900390625))
        end
    until not _G.Auto_Bone or enemy.Humanoid.Health <= 0 or not enemy.Parent or not game:GetService("Players").LocalPlayer.PlayerGui.Main.Quest.Visible
    UsefastattackPlayers = false
end